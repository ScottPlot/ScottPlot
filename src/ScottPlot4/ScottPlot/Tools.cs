...
namespace ScottPlot
{
    public static class Tools
    {
        ...
        /// <summary>
        /// Downsamples the input matrix using a 2x2 grid with simple averaging.
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static double[,] DownSampleBy2x2Grid(double[,] input)
        {
            ...
            double[] helper = new double[widthNew * heightNew];

            int indexer = 0;
            foreach (var item in ViewAsNByM(input, 2, 2))
            {
                double average = 0;
                ...
                average /= 4.0;
                helper[indexer] = average;
                indexer++;
            }

            return Convert1Dto2DArray(helper, widthNew, heightNew);
        }
        ...
        public static double?[,] DownSampleBy2x2Grid(double?[,] input)
        {
            ...
            foreach (var item in ViewAsNByM(input, 2, 2))
            {
                ...
                for (int i = 0; i < item.GetLength(0); i++)
                {
                    for (int j = 0; j < item.GetLength(1); j++)
                    {
                        if (item[i, j].HasValue) average += (double)item[i, j];
                        else divider--;
                    }
                }
                if (divider > 0) average /= divider;
                else average = null;
                helper[indexer] = average;
                indexer++;
            }
        ...
        private static T[,] Convert1Dto2DArray<T>(T[] input, int width, int height)
        {
