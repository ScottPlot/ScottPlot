...
public partial class SignalXY_ZoomBug : Form, IDemoWindow
{
    public string Title => "SignalXY Zoom Bug Demo";

    public string Description => "Demonstrates The Zoom Bug";

    public SignalXY_ZoomBug()
    {
        InitializeComponent();
    ...
    private void PopulateBuggedPlot()
    {
    ...
    private void PopulateFixedPlot()
    {
    ...
    private static void AddScatter(Plot plot)
    {
        var scatterX = Generate.Consecutive(20);
        var scatterY = Generate.SquareWave(10, 2);
        var s = plot.Add.Scatter(scatterX, scatterY, ScottPlot.Colors.Blue);
        s.Axes.XAxis = plot.Axes.Bottom;
        s.Axes.YAxis = plot.Axes.Right;
...
public class Original_SignalXYSourceDoubleArray : ISignalXYSource
{
    readonly double[] Xs;
    readonly double[] Ys;
    ...
    public Original_SignalXYSourceDoubleArray(double[] xs, double[] ys)
    {
