...
## ScottPlot 4.0 -> 4.1 Changes
Work toward ScottPlot 4.1 began in October, 2020. Improvements are focused at enhanced performance, improved thread safety, support for multiple axes, and options for data validation.
...
* Axis Limits
  ...
  * Methods which modify axis limits do not have a return value (previously they returned `double[]`)
...
* The Settings module has been greatly refactored
  * It is private, but you can request it with `Plot.GetSettings()`
...
* The Render system is stateless
...
* Signal plots can now be created with a defined `minRenderIndex` (in addition to the already-supported `maxRenderIndex`) to facilitate partial display of large arrays (#474) _Thanks @Benny121221_
